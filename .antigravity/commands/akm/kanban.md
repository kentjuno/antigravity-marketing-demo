...
if [[ "$ARGUMENTS" == *"--stop"* ]]; then
  node .antigravity/skills/plans-kanban/scripts/server.cjs --stop
  exit 0
fi
...
node .antigravity/skills/plans-kanban/scripts/server.cjs \
  --dir "$PLANS_DIR" \
  --host 0.0.0.0 \
  --open \
  --background
...
Track progress: https://github.com/antigravitykit/antigravitykit-engineer/issues/189
