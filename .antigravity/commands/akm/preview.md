...
**IMPORTANT:** Run server as Google Antigravity background task using `run_in_background: true` with the Bash tool. This makes the server visible in `/tasks` and manageable via `KillShell`.

Check if this script is located in the current workspace or in `$HOME/.antigravity/skills/markdown-novel-viewer` directory:
- If in current workspace: `$SKILL_DIR_PATH` = `./.antigravity/skills/markdown-novel-viewer/`
- If in home directory: `$SKILL_DIR_PATH` = `$HOME/.antigravity/skills/markdown-novel-viewer/`
...
```json
{
  "command": "node .antigravity/skills/markdown-novel-viewer/scripts/server.cjs --dir \"path\" --host 0.0.0.0 --open --foreground",
