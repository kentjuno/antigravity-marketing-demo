...
Think harder.
Use `skill-creator` and `antigravity-code` skills.
...
## Your mission
Update an existing skill or its reference files in `.antigravity/skills/${SKILL}` directory based on the user's prompt. 
...
### Scopes
- Project-scope: Current working project directory (e.g. `.antigravity/`)
- User-scope: Home/user directory (e.g. `~/.antigravity/`)

## IMPORTANT NOTES:
- ALWAYS make changes to skills in the project-scope `.antigravity/skills/` directory (UNLESS you're allowed to).
- DO NOT make any changes to skills in the home/user-scope `~/.antigravity/skills/` directory (UNLESS you're allowed to).
...
- Skills are not documentation, they are practical instructions for Google Antigravity to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Antigravity how to perform a specific development task, not what a tool does.
- Google Antigravity can activate multiple skills automatically to achieve the user's request.
