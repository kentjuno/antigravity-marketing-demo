...
```bash
python .antigravity/skills/ai-multimodal/scripts/gemini_batch_process.py \
  --task generate-audio \
...
Reference: `.antigravity/skills/video-production/references/audio-directive-guide.md`
...
Reference: `.antigravity/skills/ai-multimodal/references/music-generation.md`
...
```bash
node .antigravity/skills/video-production/scripts/analyze-video.cjs \
...
```bash
node .antigravity/skills/video-production/scripts/optimize-for-platform.cjs \
...
```bash
node .antigravity/skills/video-production/scripts/extract-captions.cjs \
  --video "assets/videos/{date}-{slug}/master.mp4" \
  --format srt \
...
```text
/video:create "30-second TikTok ad for AntigravityKit showing: problem -> workflow -> aha moment -> CTA"
/video:create content/video/reel/antigravitykit-workflow/script.md
/video:create assets/storyboards/antigravitykit-workflow/storyboard.json
```
