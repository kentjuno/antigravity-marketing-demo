...
```bash
node .antigravity/scripts/worktree.cjs info --json
```
...
**Monorepo:**
```bash
node .antigravity/scripts/worktree.cjs create "<PROJECT>" "<SLUG>" --prefix <TYPE> --env "<FILES>"
```

**Standalone:**
```bash
node .antigravity/scripts/worktree.cjs create "<SLUG>" --prefix <TYPE> --env "<FILES>"
```
...
Antigravity: [Runs: node .antigravity/scripts/worktree.cjs info --json]
        [Detects: standalone repo, envFiles: [".env.example"]]
        [Detects prefix from "fix" keyword: fix]
        [Converts slug: "login-validation-bug"]

Antigravity: [Uses AskUserQuestion for env files]
        "Which environment files should be copied?"
        Options: .env.example

User: .env.example

Antigravity: [Runs: node .antigravity/scripts/worktree.cjs create "login-validation-bug" --prefix fix --env ".env.example"]
