...
1. **Extract Video Data**
   ...
   - Run VidCap API:
     ```bash
     python .antigravity/skills/youtube-handling/scripts/vidcap.py info "<url>"
     python .antigravity/skills/youtube-handling/scripts/vidcap.py summary "<url>" --screenshot
     python .antigravity/skills/youtube-handling/scripts/vidcap.py caption "<url>" --ext vtt
     ```
...
3. **Design Infographic**
   ...
   - Inject brand context:
     ```bash
     node .antigravity/skills/brand-guidelines/scripts/inject-brand-context.cjs --json
     ```
   - Get embeddable design tokens (MANDATORY for standalone HTML):
     ```bash
     node .antigravity/skills/design-system/scripts/embed-tokens.cjs --minimal
     ```
   - Structure:
...
<head>
  ...
  <style>
    ...
    :root {
      /* Paste output from: node .antigravity/skills/design-system/scripts/embed-tokens.cjs --minimal */
    }
  </style>
  <style>
    body {
