 ...
 * Run: node --test .antigravity/hooks/__tests__/integration/path-resolution.test.cjs
 *
...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Subdirectory Workflow', () => {
    ...
    it('all hooks use CWD as base when in subdirectory', async () => {
      ...
      const subdirPath = path.join(gitRoot, '.antigravity', 'hooks');
      ...
      assert.ok(
        subagentContext.includes(subdirPath) || subagentContext.includes('.antigravity/hooks'),
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('subagent uses payload.cwd for path resolution', async () => {
      ...
      const subdirPath = path.join(gitRoot, '.antigravity');
      ...
      assert.ok(
        context.includes(subdirPath) || context.includes('.antigravity'),
