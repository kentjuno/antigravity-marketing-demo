 ...
 * Run: node --test .antigravity/hooks/__tests__/session-init.test.cjs
 *
...
describe('session-init.cjs', () => {
  ...
  describe('Issue #291: Git Root Path Resolution', () => {
    ...
    it('shows subdirectory info when CWD differs from git root (Issue #327)', async () => {
      ...
      // Use .antigravity/hooks as subdirectory (guaranteed to exist)
      const subdirPath = require('path').join(gitRoot, '.antigravity', 'hooks');
