...
This directory contains hooks for Google Antigravity sessions.
...
```bash
cp .antigravity/hooks/notifications/.env.example ~/.antigravity/.env
```

Edit `~/.antigravity/.env`:
```bash
...
Add to your `.antigravity/settings.json` or `.antigravity/settings.local.json`:
...
- `Notification` - Antigravity notification events (input prompts, etc.)
...
```bash
echo '{"hook_event_name":"Stop","cwd":"'$(pwd)'","session_id":"test123"}' | node .antigravity/hooks/notifications/notify.cjs
```

Expected output (if Telegram and Discord configured):
```
[env-loader] Loaded: /home/user/.antigravity/.env
...
Cross-platform hook blocking heavy directories to improve Antigravity performance.

**Blocked Patterns** (configured in `.antigravity/.ckignore`):
- `node_modules`, `__pycache__`, `.git`, `dist`, `build`

**Testing:**
```bash
echo '{"tool_input":{"command":"ls node_modules"}}' | node .antigravity/hooks/scout-block.cjs
```
...
- Verify hooks are added to `.antigravity/settings.json`
- Check Google Antigravity is using correct settings file
