 ...
 * Run: node --test .antigravity/hooks/lib/__tests__/context-builder.test.cjs
 *
...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('returns null when file does not exist anywhere', () => {
      tempDir = createTempDir(['.antigravity']);
      process.chdir(tempDir);

      // Use a unique filename that won't exist in global ~/.antigravity/
    ...
    it('finds file in rules/ directory (new location)', () => {
      tempDir = createTempDir(['.antigravity/rules']);
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.antigravity/rules/development-rules.md',
        'Should find file in rules/ directory');
    });

    it('falls back to workflows/ when rules/ does not exist', () => {
      tempDir = createTempDir(['.antigravity/workflows']);
      createTestFile(path.join(tempDir, '.antigravity/workflows'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.antigravity/workflows/development-rules.md',
        'Should fall back to workflows/ directory');
    });

    it('prefers rules/ over workflows/ when both exist', () => {
      tempDir = createTempDir(['.antigravity/rules', '.antigravity/workflows']);
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'development-rules.md', '# Rules version\n');
      createTestFile(path.join(tempDir, '.antigravity/workflows'), 'development-rules.md', '# Workflows version\n');
      process.chdir(tempDir);
    ...
    it('finds file in workflows/ when rules/ exists but file is only in workflows/', () => {
      ...
      createTestFile(path.join(tempDir, '.antigravity/workflows'), 'legacy-file.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('legacy-file.md');
      assert.strictEqual(result, '.antigravity/workflows/legacy-file.md',
  ...
  describe('resolveWorkflowPath alias', () => {
    ...
    it('resolveWorkflowPath works identically to resolveRulesPath', () => {
      const tempDir = createTempDir(['.antigravity/rules']);
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'test.md');
  ...
  describe('Global path resolution', () => {
    ...
    it('checks global ~/.antigravity/rules/ path', () => {
      ...
      const tempDir = createTempDir(['.antigravity']);
      const originalCwd = process.cwd();
      process.chdir(tempDir);
      ...
      try {
        ...
        // Result depends on whether global ~/.antigravity/rules exists
  ...
  describe('ANTIGRAVITY.md reference resolution', () => {
    let tempDir;
    let originalCwd;
    ...
    it('resolves @rules/ references correctly', () => {
      ...
      // which is used by ANTIGRAVITY.md @references
      tempDir = createTempDir(['.antigravity/rules']);
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'primary-workflow.md');
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'development-rules.md');
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'orchestration-protocol.md');
      createTestFile(path.join(tempDir, '.antigravity/rules'), 'documentation-management.md');
      process.chdir(tempDir);

      // All files referenced in ANTIGRAVITY.md should resolve
      const files = [
        'primary-workflow.md',
    ...
    it('resolves legacy @workflows/ references via fallback', () => {
      ...
      tempDir = createTempDir(['.antigravity/workflows']);
      createTestFile(path.join(tempDir, '.antigravity/workflows'), 'primary-workflow.md');
      process.chdir(tempDir);
