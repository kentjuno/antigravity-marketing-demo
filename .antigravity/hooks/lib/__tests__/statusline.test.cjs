 ...
 * Run: node .antigravity/hooks/lib/__tests__/statusline.test.cjs
 */
      ...
      content: [
        {
          ...
          id: 'agent-1',
          name: 'Task',
          input: { subagent_type: 'researcher', model: 'antigravity-opus', description: 'Research topic' }
        }
      ]
    }
  },
...
test('parseTranscript tracks agents correctly', async () => {
  ...
  assertEquals(agent.model, 'antigravity-opus', 'Should capture agent model');
});
...
test('countConfigs: returns object with expected properties', () => {
  ...
  assertTrue('antigravityMdCount' in result, 'Should have antigravityMdCount');
...
test('countConfigs: all counts are numbers', () => {
  const result = countConfigs('/tmp');
  assertEquals(typeof result.antigravityMdCount, 'number', 'antigravityMdCount should be number');
