...
// Google Antigravity statusline runs via pipe but output displays in TTY - default to true
const shouldUseColor = (() => {
  if (process.env.NO_COLOR) return false;
  if (process.env.FORCE_COLOR) return true;
  // Default true for statusline context (Google Antigravity handles TTY display)
  return true;
})();
