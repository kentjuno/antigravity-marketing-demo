 ...
 *
 * Extracted from dev-rules-reminder.cjs for reuse in both Antigravity hooks and OpenCode plugins.
 ...
 * @param {string} [configDirName='.antigravity'] - Config directory name
 * @returns {string|null} Resolved path or null
 */
function resolveRulesPath(filename, configDirName = '.antigravity') {
  // Try rules/ first (new location)
  const localRulesPath = path.join(process.cwd(), configDirName, 'rules', filename);
  const globalRulesPath = path.join(os.homedir(), '.antigravity', 'rules', filename);

  if (fs.existsSync(localRulesPath)) return `${configDirName}/rules/${filename}`;
  if (fs.existsSync(globalRulesPath)) return `~/.antigravity/rules/${filename}`;
  ...
  const globalWorkflowsPath = path.join(os.homedir(), '.antigravity', 'workflows', filename);

  if (fs.existsSync(localWorkflowsPath)) return `${configDirName}/workflows/${filename}`;
  if (fs.existsSync(globalWorkflowsPath)) return `~/.antigravity/workflows/${filename}`;
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = '.antigravity') {
  const localPath = path.join(process.cwd(), configDirName, 'scripts', filename);
  const globalPath = path.join(os.homedir(), '.antigravity', 'scripts', filename);
  if (fs.existsSync(localPath)) return `${configDirName}/scripts/${filename}`;
  if (fs.existsSync(globalPath)) return `~/.antigravity/scripts/${filename}`;
  return null;
}
 ...
 */
function resolveSkillsVenv(configDirName = '.antigravity') {
  ...
  const globalVenv = path.join(os.homedir(), '.antigravity', 'skills', '.venv', venvBin, pythonExe);
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ? '~\\.antigravity\\skills\\.venv\\Scripts\\python.exe'
      : '~/.antigravity/skills/.venv/bin/python3';
  }
  return null;
}
...
function buildRulesSection({ devRulesPath, catalogScript, skillsVenv }) {
  ...
  if (skillsVenv) {
    lines.push(`- Python scripts in .antigravity/skills/: Use \`${skillsVenv}\``);
  }
 ...
 * @param {string} [params.configDirName='.antigravity'] - Config directory name
 ...
 * }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.antigravity', baseDir } = {}) {
