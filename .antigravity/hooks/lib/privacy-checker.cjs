 ...
 *
 * Extracted from privacy-block.cjs for reuse in both Antigravity hooks and OpenCode plugins.
...
/**
 * Load .antigravity.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .antigravity.json (defaults to .antigravity in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.antigravity.json')
      : path.join(process.cwd(), '.antigravity', '.antigravity.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .antigravity.json config
