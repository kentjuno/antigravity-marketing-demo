 ...
 *
 * Extracted from session-init.cjs for reuse in both Antigravity hooks and OpenCode plugins.
...
function getCodingLevelGuidelines(level, configDir) {
  ...
  const basePath = configDir || path.join(process.cwd(), '.antigravity');
...
function buildStaticEnv(configDir) {
  return {
    ...
    locale: process.env.LANG || '',
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
    configDir: configDir || path.join(process.cwd(), '.antigravity')
  };
}
