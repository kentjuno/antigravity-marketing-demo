 ...
 * Antigravity hooks and OpenCode plugins.
 *
 ...
 * @param {string} [params.options.antigravityDir] - Path to .antigravity or .opencode directory
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  const {
    ckignorePath,
    antigravityDir = path.join(process.cwd(), '.antigravity'),
  ...
  const resolvedCkignorePath = ckignorePath || path.join(antigravityDir, '.ckignore');
