...
Get Discord notifications when Google Antigravity sessions complete.
...
Add to your `.antigravity/settings.json` (project) or `~/.antigravity/settings.json` (global):
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node .antigravity/hooks/notifications/notify.cjs"
          }
        ]
      }
    ]
...
The original bash scripts (`discord_notify.sh`, `telegram_notify.sh`) are **deprecated** due to jq PATH issues in Google Antigravity's subprocess environment. Use `notify.cjs` instead.

---

## Overview

The Discord hook (`send-discord.sh`) sends rich embedded messages to a Discord channel when Google Antigravity completes implementation tasks. Messages include session time, project info, and task summaries.
...
3. Click **"New Webhook"**
4. Configure webhook:
   - **Name:** `Google Antigravity Bot` (or your preference)
...
2. **.antigravity/.env** - Project-level Antigravity configuration
3. **.antigravity/hooks/.env** - Hook-specific configuration
...
**Option B: `.antigravity/.env`** (project-level override):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```

**Option C: `.antigravity/hooks/.env`** (hook-specific):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```
...
```bash
chmod +x .antigravity/hooks/send-discord.sh
```
...
```bash
./.antigravity/hooks/send-discord.sh 'Test notification from Google Antigravity'
```
...
```bash
./.antigravity/hooks/send-discord.sh 'Task completed: Added user authentication feature'
```

**With multi-line messages:**
```bash
./.antigravity/hooks/send-discord.sh 'Implementation Complete
...
```bash
./.antigravity/hooks/send-discord.sh 'Fixed bug in user'\''s profile page'
```

### Automated Usage (Google Antigravity Workflow)

Antigravity automatically calls this script when completing implementations. This is configured in `.antigravity/rules/development-rules.md`:

```markdown
- When you finish the implementation, send a full summary report to Discord channel
  with `./.antigravity/hooks/send-discord.sh 'Your message here'` script
```
...
if npm run build && npm run test; then
    ./.antigravity/hooks/send-discord.sh '✅ Build and tests passed - ready to deploy'
else
    ./.antigravity/hooks/send-discord.sh '❌ Build or tests failed - deployment blocked'
fi
```
...
{
  "scripts": {
    "deploy": "npm run build && npm run test && ./.antigravity/hooks/send-discord.sh '✅ Deployment successful'",
    "notify": "./.antigravity/hooks/send-discord.sh"
  }
}
```
...
**Embed Components:**
- **Title:** 🤖 Google Antigravity Session Complete
...
```
╔═══════════════════════════════╗
║ 🤖 Google Antigravity Session Complete
╠═══════════════════════════════╣
...
║ 📂 Project: antigravitykit-engineer
╠═══════════════════════════════╣
║ Project Name • antigravitykit-engineer
║ Today at 14:30
╚═══════════════════════════════╝
```
...
```bash
./.antigravity/hooks/send-discord.sh 'Message with "double quotes" works fine'
```

**Escape single quotes inside single-quoted strings:**
```bash
./.antigravity/hooks/send-discord.sh 'User'\''s profile updated'
```

**Use double quotes and escape:**
```bash
./.antigravity/hooks/send-discord.sh "Message with \"escaped quotes\""
```

**For complex messages, use heredoc:**
```bash
./.antigravity/hooks/send-discord.sh "$(cat <<'EOF'
Multi-line message
...
./.antigravity/hooks/send-discord.sh "$1"
...
if npm run test; then
    if [[ "$ENVIRONMENT" == "production" ]]; then
        ./.antigravity/hooks/send-discord.sh '✅ Production deployment successful'
    fi
else
    # Always notify on failures
    ./.antigravity/hooks/send-discord.sh '❌ Tests failed - deployment aborted'
fi
```
...
**Script Location:** `.antigravity/hooks/send-discord.sh`
...
**Google Antigravity Documentation:** https://docs.antigravity.com/antigravity-code
