...
Get Telegram notifications when Google Antigravity sessions complete.
...
The original `telegram_notify.sh` is **deprecated** due to jq PATH issues in Google Antigravity's subprocess environment. Use `notify.cjs` instead.

---

## Overview

The Telegram hook (`telegram_notify.sh`) automatically sends notifications when Google Antigravity sessions stop or subagents complete tasks. It provides detailed summaries including tool usage, files modified, and operation counts.
...
3. Follow the prompts:
   ...
   You: Google Antigravity Notifier

   BotFather: Good. Now let's choose a username for your bot.
   You: antigravitycode_notifier_bot
   ```
...
#### Option C: `.antigravity/.env` (Project-Level Override)

For project-specific Antigravity configuration:
...
#### Option D: `.antigravity/hooks/.env` (Hook-Specific)
...
### 4. Configure Google Antigravity Hook

Hooks are configured in `.antigravity/settings.local.json`:
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "${CLAUDE_PROJECT_DIR}/.antigravity/hooks/telegram_notify.sh"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
- `"Stop"`: Triggers when main Google Antigravity session ends
...
```bash
chmod +x .antigravity/hooks/telegram_notify.sh
```
...
}' | ./.antigravity/hooks/notifications/telegram_notify.sh
```

> **Note:** Google Antigravity hooks use snake_case field names. The `Stop` hook does not include tool usage data.

**Expected output:**
```
Telegram notification sent for Stop event in project antigravitykit-engineer
```
...
**Triggered when:** Main Google Antigravity session ends (user stops Antigravity or task completes)
...
📍 Location: `/Users/user/projects/antigravitykit-engineer`
```
...
**Cause:** Google Antigravity hook configuration incorrect or hook script not executable

**Solutions:**

1. **Verify `.antigravity/config.json` exists and is valid JSON:**
   ```bash
   cat .antigravity/config.json | jq .
   ```

2. **Check hook configuration:**
   ```bash
   cat .antigravity/config.json | jq '.hooks'
   ```

3. **Verify script is executable:**
   ```bash
   ls -l .antigravity/hooks/telegram_notify.sh
...
> **Note:** Google Antigravity hooks use snake_case field names per the official API.
...
**Script Location:** `.antigravity/hooks/telegram_notify.sh`
