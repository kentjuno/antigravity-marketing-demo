 ...
 *
 * Blocks access to directories listed in .antigravity/.ckignore
 ...
 *
 * Configuration:
 * - Edit .antigravity/.ckignore to customize blocked patterns (one per line, # for comments)
...
// Import formatters (kept local as they're Antigravity-specific output)
...
try {
  ...
  const antigravityDir = path.dirname(__dirname); // Go up from hooks/ to .antigravity/
  ...
  const result = checkScoutBlock({
    ...
    options: {
      antigravityDir,
      ckignorePath: path.join(antigravityDir, '.ckignore'),
