 ...
 *
 * @param {string} antigravityDir - Path to .antigravity directory
 * @returns {string}
 */
function formatConfigPath(antigravityDir) {
  if (antigravityDir) {
    return path.join(antigravityDir, '.ckignore');
  }
  return '.antigravity/.ckignore';
}
 ...
 * @param {string} details.antigravityDir - Path to .antigravity directory
 * @returns {string}
 */
function formatBlockedError(details) {
  const { path: blockedPath, pattern, tool, antigravityDir } = details;
  const configPath = formatConfigPath(antigravityDir);
