...
async function main() {
  try {
    ...
    // Extract task list ID for Google Antigravity Tasks coordination (shared helper)
    ...
    const staticEnv = {
      ...
      antigravitySettingsDir: path.resolve(__dirname, '..')
    };
    ...
    if (envFile) {
      ...
      // Google Antigravity Tasks integration - enables multi-session/subagent coordination
      ...
      writeEnv(envFile, 'CK_CLAUDE_SETTINGS_DIR', staticEnv.antigravitySettingsDir);
    ...
    // This warning reminds Antigravity to verify if user approval was pending before proceeding.
    // Upstream bug: Google Antigravity CLI should preserve pending interactive state during compaction.
    if (source === 'compact') {
