...
const CK_CONFIG_PATH = path.join(__dirname, '..', '.antigravity.json');
...
async function main() {
  ...
  // Config toggle tests - requires temp directory with .antigravity/.antigravity.json
  console.log('\n\x1b[1m--- Config Toggle (privacyBlock setting) ---\x1b[0m');
  const os = require('os');
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-test-'));
  const tmpAntigravityDir = path.join(tmpDir, '.antigravity');
  fs.mkdirSync(tmpAntigravityDir, { recursive: true });

  for (const test of configToggleTests) {
    // Write test config
    fs.writeFileSync(
      path.join(tmpAntigravityDir, '.antigravity.json'),
