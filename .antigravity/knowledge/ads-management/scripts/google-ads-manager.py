...
_antigravity_scripts = _scripts_dir.parents[2] / 'scripts'
if _antigravity_scripts.exists():
    sys.path.insert(0, str(_antigravity_scripts))
...
def _resolve(var_name: str) -> str:
    ...
    if load_dotenv:
        ...
        antigravity_dir = skills_dir.parent
        for env_file in [antigravity_dir / '.env', skills_dir / '.env', skill_dir / '.env']:
            if env_file.exists():
