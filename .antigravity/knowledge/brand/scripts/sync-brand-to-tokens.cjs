...
const GENERATE_TOKENS_SCRIPT = '.antigravity/skills/design-system/scripts/generate-tokens.cjs';
...
function updateDesignTokens(tokens, colors) {
  // Update brand name
  const brandName = `AntigravityKit Marketing - ${colors.primary.name.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}`;
