...
def load_env():
    ...
    env_paths = [
        ...
        Path.home() / ".antigravity" / "skills" / ".env",
        Path.home() / ".antigravity" / ".env"
    ]
...
def cheag_logo_required(brand_name, skip_prompt=False):
...
Image Editing Mode:
  ...
  to incorporate your ACTUAL logo into the CIP mockups.
    ...
    if args.logo:
        ...
        action = cheag_logo_required(args.brand, skip_prompt=args.no_logo_prompt)
        if action == 'generate':
            print("\n💡 To generate a logo, use the logo-design skill:")
            print(f"   python ~/.antigravity/skills/logo-design/scripts/generate.py --brand \"{args.brand}\" --industry \"{args.industry}\"")
