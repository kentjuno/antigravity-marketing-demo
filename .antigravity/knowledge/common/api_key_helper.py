...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    antigravity_env = project_dir / '.antigravity' / '.env'
    if antigravity_env.exists():
        api_key = load_env_file(antigravity_env)
        if api_key:
            print(f"✓ Using API key from {antigravity_env}", file=sys.stderr)
            return api_key

    # Step 4: Check ./.antigravity/skills/.env
    antigravity_skills_env = project_dir / '.antigravity' / 'skills' / '.env'
    if antigravity_skills_env.exists():
        api_key = load_env_file(antigravity_skills_env)
        if api_key:
            print(f"✓ Using API key from {antigravity_skills_env}", file=sys.stderr)
            return api_key
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3️⃣  .antigravity/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.antigravity/.env", file=sys.stderr)

        print("\n4️⃣  .antigravity/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
