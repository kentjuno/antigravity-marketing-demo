...
Invoke `project-management` skill to create persistent TODO tasks via Antigravity's native task orchestration. Break down into:
...
1. **`/akm:brand`** — Extract brand colors, fonts, voice from user's project
2. **`/akm:design-system`** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** `/ag:ui-ux-pro-max` OR `/ag:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
