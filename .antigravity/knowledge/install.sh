...
# Installs all dependencies for Google Antigravity skills
#
...
# Print functions (must be defined before cheag_bash_version)
print_header() {
    echo -e "\n${BLUE}===================================================${NC}"
    echo -e "${BLUE}$1${NC}"
    echo -e "${BLUE}===================================================${NC}\n"
...
install_system_package() {
    ...
    local cheag_commands="$3"  # Comma-separated commands to check

    # Check if already installed (check multiple commands)
    IFS=',' read -ra cmds <<< "$cheag_commands"
    for cmd in "${cmds[@]}"; do
        if command_exists "$cmd"; then
            print_success "$display_name already installed"
            traag_success "optional" "$display_name"
            return 0
        fi
    done
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        if brew install "$package_name" 2>/dev/null; then
            ...
            traag_failure "optional" "$display_name" "brew install failed"
            return 1
        fi
    fi
...
print_usage() {
    ...
    echo -e "  source .antigravity/skills/.venv/bin/activate"
    echo ""
    echo -e "${BLUE}For more information, see:${NC}"
    echo -e "  .antigravity/skills/INSTALLATION.md"
    echo ""
}
...
main() {
    ...
    print_header "Google Antigravity Skills Installation"
    print_info "OS: $OS"
