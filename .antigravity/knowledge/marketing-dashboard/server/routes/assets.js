...
const app = new Hono();
// Mock DB object to bypass better-sqlite3 build failures in Local-First UI
const db = {
  prepare: () => ({
    all: () => [],
    get: () => null,
...
// GET /api/assets/:id/content - Get raw content (live scan, no DB required)
app.get('/:id/content', async (c) => {
  try {
    const { id } = c.req.param();

    // Find asset via live scan (bypass mock DB)
    const assetsDir = join(__dirname, '../../../../../assets');
    const manifest = scanAssets(assetsDir, null, { update: false });
    ...
    const filePath = resolve(join(assetsRoot, asset.path.replace(/[\/\\]/g, sep)));

    // Security: must stay within assets directory
    ...
    return c.json({ content, format: asset.format, size: content.length });
  } catch (error) {
...
// GET /api/assets/:id - Get single asset (live scan fallback)
app.get('/:id', (c) => {
  try {
    const { id } = c.req.param();
    // Skip if this looks like a sub-route handled elsewhere
    if (id === 'scan' || id === 'file') return c.json({ error: 'Not found' }, 404);
