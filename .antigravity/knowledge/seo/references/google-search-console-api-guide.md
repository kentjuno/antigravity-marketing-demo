...
## Quick Start (AntigravityKit Scripts)

AntigravityKit provides cross-platform scripts for GSC integration. Use these for most tasks:
...
**Config location:** `.antigravity/secrets/google_client_secret.json` (project) or `~/.antigravity/secrets/` (global)
...
8. Place in .antigravity/secrets/ (project scope) or ~/.antigravity/secrets/ (global)
```
...
**Step 3: Authenticate with AntigravityKit Script**
```bash
