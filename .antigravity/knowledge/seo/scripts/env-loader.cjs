#!/usr/bin/env node
/**
 * Environment Variable Loader for Google Antigravity Skills
 *
 * Loads environment variables following the priority order:
 * 1. process.env (highest priority - already set)
 * 2. $HOME/.antigravity/skills/${SKILL}/.env (skill-specific)
 * 3. $HOME/.antigravity/skills/.env (shared across skills)
 * 4. $HOME/.antigravity/.env (global Antigravity config)
 *
 * Usage:
...
function loadEnv(skillName) {
  ...
  const envPaths = [
    path.join(homeDir, '.antigravity', '.env'), // 4. Global Antigravity config (lowest)
    path.join(homeDir, '.antigravity', 'skills', '.env'), // 3. Shared skills
  ];

  // Add skill-specific .env if skill name provided
  if (skillName) {
    envPaths.push(
      path.join(homeDir, '.antigravity', 'skills', skillName, '.env') // 2. Skill-specific
    );
  }
 ...
 * Priority: process.env > skill .env > skills .env > antigravity .env
