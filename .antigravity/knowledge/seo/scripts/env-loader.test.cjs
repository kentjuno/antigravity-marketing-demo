...
function setup() {
  ...
  fs.mkdirSync(path.join(tempDir, '.antigravity'), { recursive: true });
  fs.mkdirSync(path.join(tempDir, '.antigravity', 'skills'), { recursive: true });
  fs.mkdirSync(path.join(tempDir, '.antigravity', 'skills', 'test-skill'), {
    recursive: true,
  });

  // Create .env files with different values to test priority
  fs.writeFileSync(
    path.join(tempDir, '.antigravity', '.env'),
    'GLOBAL_VAR=global\nSHARED_VAR=global\nSKILL_VAR=global\n'
  );

  fs.writeFileSync(
    path.join(tempDir, '.antigravity', 'skills', '.env'),
    'SHARED_VAR=shared\nSKILL_VAR=shared\n'
  );

  fs.writeFileSync(
    path.join(tempDir, '.antigravity', 'skills', 'test-skill', '.env'),
