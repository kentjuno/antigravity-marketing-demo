 ...
 * 1. Project-scope: .antigravity/secrets/google_client_secret.json
 * 2. Global (Unix): ~/.antigravity/secrets/google_client_secret.json
 * 3. Global (Windows): %USERPROFILE%\.antigravity\secrets\google_client_secret.json
 *
...
/**
 * Get project root (searches upward for .antigravity directory)
 */
function getProjectRoot() {
  let dir = process.cwd();
  while (dir !== path.dirname(dir)) {
    if (fs.existsSync(path.join(dir, '.antigravity'))) {
      return dir;
    }
...
function getConfigPaths() {
  ...
  return [
    ...
    path.join(projectRoot, '.antigravity', SECRETS_DIR, CONFIG_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.antigravity', SECRETS_DIR, CONFIG_FILENAME),
  ];
}
...
function getTokenPaths() {
  ...
  return [
    // 1. Project-scope (highest priority)
    path.join(projectRoot, '.antigravity', SECRETS_DIR, TOKEN_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.antigravity', SECRETS_DIR, TOKEN_FILENAME),
  ];
}
...
Searched paths (priority order):
  1. PROJECT/.antigravity/secrets/google_client_secret.json
  2. ~/.antigravity/secrets/google_client_secret.json (or %USERPROFILE% on Windows)
`);
  } else {
