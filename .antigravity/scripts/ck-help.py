#!/usr/bin/env python3
"""
AntigravityKit Help Command - All-in-one guide with dynamic command discovery.
Scans .antigravity/commands/ directory to build catalog at runtime.
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ck.json", ".antigravity.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .antigravity.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "AntigravityKit Configuration (.antigravity.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.antigravity/.antigravity.json`"),
            ("Local", "Override per-project in `./.antigravity/.antigravity.json`"),
    ...
    "notifications": {
        ...
        "workflow": [
            ("1. Set env vars", "Add `DISCORD_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID` to `~/.antigravity/.env`"),
            ("2. Add hook", "Add Stop hook to `.antigravity/settings.json` (see below)"),
            ("3. Test", "`echo '{\"hook_event_name\":\"Stop\"}' | node .antigravity/hooks/notifications/notify.cjs`"),
        ],
        "tip": """Add to settings.json:
```json
"Stop": [{"matcher": "*", "hooks": [{"type": "command", "command": "node .antigravity/hooks/notifications/notify.cjs"}]}]
```
Docs: `.antigravity/hooks/notifications/docs/`""",
    },
}


def detect_prefix(commands_dir: Path) -> str:
    """Detect if commands use /ag: prefix based on directory structure."""
    ag_commands_dir = commands_dir / "ck"
    return "ck:" if ag_commands_dir.exists() and ag_commands_dir.is_dir() else ""
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .antigravity/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_command(data: dict, command: str, prefix: str) -> None:
    ...
    search = command.lower().replace("/ag:", "").replace("/", "").replace(":", "")

    found = None
    for cmds in commands.values():
        for cmd in cmds:
            # Normalize command name for comparison
            name = cmd["name"].lower().replace("/ag:", "").replace("/", "").replace(":", "")
            if name == search:
                found = cmd
                break
        if found:
...
def show_config_guide() -> None:
    """Display comprehensive .antigravity.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# AntigravityKit Configuration (.antigravity.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.antigravity/.antigravity.json` (user preferences)")
    print("- Local: `./.antigravity/.antigravity.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.antigravity/.antigravity.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.antigravity/.antigravity.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print()
    print("When both are set, Antigravity thinks in one language but responds in another.")
    ...
    print("```bash")
    print("# ~/.antigravity/.antigravity.json - applies everywhere")
    print("cd /tmp/new-project && antigravity  # Uses global config")
    print("```")
    print()
    print("**Project with local override:**")
    print("```bash")
    print("# Global: issuePrefix = \"GH-\"")
    print("# Local (.antigravity/.antigravity.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("# Coding Level (Adaptive Communication)")
    print()
    print("Adjusts Antigravity's communication style based on user's experience level.")
    ...
    print("- Global: `~/.antigravity/.antigravity.json` - personal preference")
    print("- Local: `./.antigravity/.antigravity.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.antigravity.json`")
    print("2. If 0-5, injects guidelines from `.antigravity/output-styles/coding-level-*.md`")
    ...
    print("## Customization")
    print()
    print("Guidelines live in `.antigravity/output-styles/coding-level-*.md`")
...
def main():
    # Find .antigravity/commands directory
    script_path = Path(__file__).resolve()
    antigravity_dir = script_path.parent.parent  # .antigravity/scripts -> .antigravity
    commands_dir = antigravity_dir / "commands"

    if not commands_dir.exists():
        print("Error: .antigravity/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .antigravity/commands/")
        sys.exit(1)
    ...
    if input_str.lower() in ["config", "configuration", ".antigravity.json", "ck.json"]:
        show_config_guide()
        return
