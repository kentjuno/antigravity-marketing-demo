#!/usr/bin/env python3
"""
Scan .antigravity/skills directory and extract skill metadata.
"""
...
def scan_skills(base_path: Path) -> List[Dict]:
    ...
    for skill_file in sorted(base_path.rglob('SKILL.md')):
        ...
        try:
            ...
            skills.append({
                'name': skill_name,
                'path': str(skill_file.relative_to(Path('.antigravity/skills'))),
...
def main():
    """Main execution."""
    base_path = Path('.antigravity/skills')
    ...
    output_path = Path('.antigravity/scripts/skills_data.yaml')
