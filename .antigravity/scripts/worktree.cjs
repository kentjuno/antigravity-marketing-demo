#!/usr/bin/env node
/**
 * Git Worktree Manager for AntigravityKit
 ...
 *   --worktree-root <path> Explicit worktree directory (Antigravity's decision)
...
// --worktree-root: explicit override for worktree location (Antigravity's decision)
...
function getWorktreeRoot(gitRoot, isMonorepo, explicitRoot = null) {
  // Priority 0: Explicit --worktree-root flag (Antigravity's decision)
  if (explicitRoot) {
...
function cmdCreate() {
  ...
  // explicitWorktreeRoot comes from --worktree-root flag (Antigravity's decision)
  ...
  if (fs.existsSync(worktreePath)) {
    outputError('WORKTREE_EXISTS', `Worktree already exists: ${worktreePath}`, {
      suggestion: `To use: cd ${worktreePath} && antigravity\nTo remove: git worktree remove ${worktreePath}`
    });
  }
