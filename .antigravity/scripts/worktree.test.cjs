 ...
 * Run: node .antigravity/scripts/worktree.test.cjs
 */
...
const MONOREPO_DIR = '/home/kai/antigravitykit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'antigravitykit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from antigravitykit-engineer submodule
  const submodulePath = '/home/kai/antigravitykit/antigravitykit-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'antigravitykit-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'antigravitykit-cli', 'Should find antigravitykit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'antigravitykit' matches multiple projects
  const result = run('create antigravitykit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
