...
/**
 * Custom Google Antigravity statusline for Node.js - Multi-line Edition
...
function renderSessionLines(ctx) {
  ...
  // Add usage/reset info to session part (stays on line 1 with model - Google Antigravity only reads line 1)
  ...
  // Layout priority: SESSION FIRST (Google Antigravity only reads line 1)
  // Line 1: model + context + usage (most important for Google Antigravity)
...
async function main() {
  try {
    ...
    const modelName = data.model?.display_name || 'Antigravity';
