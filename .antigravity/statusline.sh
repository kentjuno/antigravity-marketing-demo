#!/bin/bash
# Custom Google Antigravity statusline for Bash
...
if command -v jq >/dev/null 2>&1; then
  ...
  model_name=$(echo "$input" | jq -r '.model.display_name // "Antigravity"' 2>/dev/null)
  ...
  [ -z "$model_name" ] && model_name="Antigravity"
  model_version=""
...
# Extract context window usage (Google Antigravity v2.0.65+)
...
if [ -n "$transcript_path" ] && [ -f "$transcript_path" ]; then
  ...
  if [ -n "$first_api_call" ]; then
    ...
    bloag_start=$((10#$current_utc_hour / 5 * 5))
    bloag_end=$((bloag_start + 5))

    # Handle day wraparound (e.g., 20:00 UTC block ends at 01:00 UTC next day)
    if [ $bloag_end -ge 24 ]; then
      bloag_end=$((bloag_end - 24))
      bloag_end_date="tomorrow"
    else
      bloag_end_date="today"
    fi

    # Calculate remaining time until block reset
    now_sec=$(date +%s)
    bloag_end_sec=$(date -u -d "${bloag_end_date} ${bloag_end}:00 UTC" +%s 2>/dev/null)

    if [ -n "$bloag_end_sec" ] && [ "$bloag_end_sec" -gt 0 ]; then
      remaining=$((bloag_end_sec - now_sec))

      if [ $remaining -gt 0 ] && [ $remaining -lt 18000 ]; then
        rh=$((remaining / 3600))
        rm=$(((remaining % 3600) / 60))
        bloag_end_local=$(date -d "@${bloag_end_sec}" +"%H:%M" 2>/dev/null)
        session_txt="${rh}h ${rm}m until reset at ${bloag_end_local}"
      fi
    fi
  fi
fi
...
# context window usage (Google Antigravity v2.0.65+)
