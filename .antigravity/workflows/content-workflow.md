...
**IMPORTANT:** Antigravity MUST automatically run `/write/audit` after any content generation command (`/youtube:*`, `/write:*`) and apply `/write/publish` fixes before presenting final output.
...
# Antigravity auto-triggers workflow:
...
**User doesn't need to know commands.** Antigravity picks the right ones based on intent.
