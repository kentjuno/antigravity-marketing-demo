...
Generates .opencode directory structure and AGENTS.md from Google Antigravity configuration.
Converts Google Antigravity agents and commands to OpenCode-compatible format.
...
def find_project_root() -> Path:
    """Find project root by looking for .git or ANTIGRAVITY.md."""
    current = Path.cwd()
    for parent in [current] + list(current.parents):
        if (parent / ".git").exists() or (parent / "ANTIGRAVITY.md").exists():
            return parent
    return current


def replace_antigravity_paths_in_file(file_path: Path) -> None:
    """Replace .antigravity/ paths with .opencode/ in a file."""
    try:
        content = file_path.read_text(encoding="utf-8")
        if ".antigravity/" in content:
            content = content.replace(".antigravity/", ".opencode/")
            file_path.write_text(content, encoding="utf-8")
    except (UnicodeDecodeError, PermissionError):
        pass  # Skip binary files or files we can't access


def replace_antigravity_paths_in_dir(dir_path: Path) -> None:
    """Replace .antigravity/ paths with .opencode/ in markdown and Python files."""
    for pattern in ["*.md", "*.py"]:
        for file in dir_path.rglob(pattern):
            replace_antigravity_paths_in_file(file)
                    ...
                    if value in (">-", ">", "|-", "|"):
                        ...
                        while i < len(lines):
                            ...
                            if next_line.startswith("  ") or next_line.startswith("\t"):
                                bloag_lines.append(next_line.strip())
                                i += 1
...
def convert_antigravity_agent_to_opencode(antigravity_agent: dict, body: str, agent_name: str) -> tuple[dict, str]:
    """Convert Google Antigravity agent format to OpenCode agent format.
    ...
    opencode_frontmatter = {
        "description": antigravity_agent.get("description", f"Agent: {agent_name}"),
        "mode": mode,
    }
    ...
    # Replace .antigravity/ paths with .opencode/ in body
    body = body.replace(".antigravity/", ".opencode/")

    return opencode_frontmatter, body


def convert_antigravity_command_to_opencode(antigravity_cmd: dict, body: str, cmd_name: str) -> tuple[dict, str]:
    """Convert Google Antigravity command format to OpenCode command format."""
    opencode_frontmatter = {
        "description": antigravity_cmd.get("description", f"Command: {cmd_name}"),
    }

    # Map agent if specified
    if "agent" in antigravity_cmd:
        opencode_frontmatter["agent"] = antigravity_cmd["agent"]

    # OpenCode uses $ARGUMENTS instead of Antigravity's $ARGUMENTS (same format)
    # Convert argument-hint to description if more descriptive
    if "argument-hint" in antigravity_cmd:
        hint = antigravity_cmd["argument-hint"]
...
def generate_agents_md(project_root: Path) -> str:
    ...
    antigravity_md_path = project_root / "ANTIGRAVITY.md"
    ...
    # Extract key info from ANTIGRAVITY.md if exists
    antigravity_instructions = ""
    if antigravity_md_path.exists():
        with open(antigravity_md_path, "r", encoding="utf-8") as f:
            antigravity_md = f.read()
            # Extract workflows section
            if "## Workflows" in antigravity_md:
                start = antigravity_md.index("## Workflows")
                end = antigravity_md.find("\n## ", start + 1)
                if end == -1:
                    end = len(antigravity_md)
                antigravity_instructions = antigravity_md[start:end].strip()
...
{antigravity_instructions if antigravity_instructions else '''## Workflows
...
*Generated by AntigravityKit OpenCode Generator*
 ...
 *
 * Equivalent to Antigravity's privacy-block.cjs hook.
  ...
  return {
    "tool.execute.before": async (input: any, output: any) => {
      const result = checkScoutBlock({
        toolName: input.tool,
        toolInput: output.args,
        options: { ckignorePath, antigravityDir }
      });
...
PLUGIN_PACKAGE_JSON = '''{
  "name": "@antigravitykit/opencode-plugins",
  "version": "1.0.0",
  "description": "AntigravityKit hooks converted to OpenCode plugins",
  "dependencies": {
...
def generate_opencode_plugins(
    project_root: Path, antigravity_dir: Path, opencode_dir: Path, args
) -> None:
    """Generate OpenCode TypeScript plugins from Antigravity hooks."""
    plugin_dir = opencode_dir / "plugin"
    lib_dir = plugin_dir / "lib"

    # Check if hooks/lib exists
    hooks_lib_dir = antigravity_dir / "hooks" / "lib"
    if not hooks_lib_dir.exists():
        if args.verbose:
            print("\nSkipped plugins: .antigravity/hooks/lib/ not found")
        return
        ...
        if src.exists():
            ...
            if not dst.exists() or args.force:
                if args.dry_run:
                    ...
                    # Replace .antigravity/ paths with .opencode/ in lib file
                    replace_antigravity_paths_in_file(dst)
                    if args.verbose:
    ...
    scout_bloag_dir = antigravity_dir / "hooks" / "scout-block"
    scout_dst_dir = plugin_dir / "scout-block"
    if scout_bloag_dir.exists():
        if not scout_dst_dir.exists() or args.force:
            if args.dry_run:
                ...
                shutil.copytree(scout_bloag_dir, scout_dst_dir)
                # Replace .antigravity/ paths with .opencode/ in scout-block files
                replace_antigravity_paths_in_dir(scout_dst_dir)
                if args.verbose:
                    print(f"  Copied scout-block/ modules")

    # 3. Copy .ckignore to .opencode/
    ckignore_src = antigravity_dir / ".ckignore"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate OpenCode configuration from Google Antigravity setup"
    )
    ...
    # Convert Google Antigravity agents to OpenCode agents
    antigravity_agents_dir = antigravity_dir / "agents"
    if antigravity_agents_dir.exists():
        print("\nConverting agents...")
        for agent_file in antigravity_agents_dir.glob("*.md"):
            ...
            opencode_fm, opencode_body = convert_antigravity_agent_to_opencode(
        ...
        print(f"  Converted {len(list(antigravity_agents_dir.glob('*.md')))} agents")

    # Convert Google Antigravity commands to OpenCode commands
    ...
    antigravity_commands_dir = antigravity_dir / "commands"
    if antigravity_commands_dir.exists():
        print("\nConverting commands...")
        converted_count = 0

        # Use rglob to find all .md files including nested directories
        for cmd_file in antigravity_commands_dir.rglob("*.md"):
            # Get relative path from commands dir and flatten with dashes
            rel_path = cmd_file.relative_to(antigravity_commands_dir)
            ...
            frontmatter, body = parse_yaml_frontmatter(content)
            opencode_fm, opencode_body = convert_antigravity_command_to_opencode(
    ...
    # Copy skills from .antigravity/skills/ to .opencode/skills/
    antigravity_skills_dir = antigravity_dir / "skills"
    opencode_skills_dir = opencode_dir / "skills"
    if antigravity_skills_dir.exists():
        print("\nCopying skills...")
        skill_count = 0
        for skill_dir in antigravity_skills_dir.iterdir():
            if skill_dir.is_dir() and (skill_dir / "SKILL.md").exists():
                ...
                if args.dry_run:
                    ...
                    # Replace .antigravity/ paths with .opencode/ in skill markdown files
                    replace_antigravity_paths_in_dir(target_dir)
                    if args.verbose:
                        print(f"  Copied: {skill_name}")
                skill_count += 1
        print(f"  Copied {skill_count} skills")

    # Copy workflows from .antigravity/workflows/ to .opencode/workflow/
    antigravity_workflows_dir = antigravity_dir / "workflows"
    opencode_workflows_dir = opencode_dir / "workflows"
    if antigravity_workflows_dir.exists():
        print("\nCopying workflows...")
        ...
        workflow_count = 0
        for workflow_file in antigravity_workflows_dir.glob("*.md"):
            ...
            if args.dry_run:
                ...
                # Replace .antigravity/ paths with .opencode/ in workflow file
                replace_antigravity_paths_in_file(target_path)
                if args.verbose:
                    print(f"  Copied: {workflow_name}")
            workflow_count += 1
        print(f"  Copied {workflow_count} workflows")

    # Copy scripts from .antigravity/scripts/ to .opencode/scripts/
    antigravity_scripts_dir = antigravity_dir / "scripts"
    opencode_scripts_dir = opencode_dir / "scripts"
    if antigravity_scripts_dir.exists():
        print("\nCopying scripts...")
        if opencode_scripts_dir.exists() and not args.force:
            ...
            if args.dry_run:
                ...
                shutil.copytree(antigravity_scripts_dir, opencode_scripts_dir)
                # Replace .antigravity/ paths with .opencode/ in script files
                replace_antigravity_paths_in_dir(opencode_scripts_dir)
                script_count = len(list(opencode_scripts_dir.glob("*")))
                print(f"  Copied {script_count} scripts")

    # Copy .env.example if exists
    env_example_src = antigravity_dir / ".env.example"
    ...
    # Generate OpenCode plugins from Antigravity hooks
    generate_opencode_plugins(project_root, antigravity_dir, opencode_dir, args)
