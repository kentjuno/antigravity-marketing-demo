#!/usr/bin/env node
/**
 * Generate release manifest for AntigravityKit with file timestamps
...
const INCLUDE_DIRS = ['.antigravity', '.opencode', 'plans/templates'];
const INCLUDE_FILES = ['.gitignore', '.repomixignore', '.mcp.json', 'ANTIGRAVITY.md', 'AGENTS.md'];
...
function main() {
  ...
  for (const file of allFiles) {
    try {
      ...
      // Strip .antigravity/ prefix for files inside .antigravity directory
      // CLI tracks files relative to .antigravity/, not project root
      if (relativePath.startsWith('.antigravity/')) {
        relativePath = relativePath.slice('.antigravity/'.length);
      }
