...
(function main() {
  ...
  const metadataPath = path.join(antigravityDir, 'metadata.json');
  const distDir = path.join(projectRoot, 'dist');
  const archivePath = path.join(distDir, 'antigravitykit-marketing.zip');
  const manifestPath = path.join(projectRoot, 'release-manifest.json');

  // Critical files that MUST be present in release
  const CRITICAL_TARGETS = ['.antigravity', '.opencode', 'release-manifest.json'];
  ...
  try {
    ...
    // Generate OpenCode configuration from Google Antigravity setup
